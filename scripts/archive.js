...
import {ArchiveResultFormatterTable, ArchiveResultFormatterBox, ArchiveResultFormatterRowBox, ArchiveResultFormatterOaksTable} from './ArchiveResultFormatter.js';
...
var ArchiveProgram2 = {
	...
	Start: function() {
		this.page = document.querySelector('#page-archive2');
		if (!this.page) {
			throw "The page structure has been broken.";
		}
		...
		document.querySelectorAll('#navbar button').forEach(item => {
			item.addEventListener('click', event => {
				if (item.dataset.first !== undefined) {
					document.getElementById('page-archive').classList.add('hidden');
					document.getElementById('page-archive2').classList.remove('hidden');
				} else if (item.dataset.second !== undefined) {
					document.getElementById('page-archive').classList.remove('hidden');
					document.getElementById('page-archive2').classList.add('hidden');
				}
			});
		});
	...
	SubmitHandler: function () {
		...
		let searchResultsContainer = this.page.querySelector('.search-results-content');
		...
		try {
			...
			this.Ajax({
				url: `${this.searchForm.action}&q2=${encodeURIComponent(searchInput.value)}`,
				success: (response) => {
					if (response.error) {
						//this.searchFeedback.innerText = response.error;
